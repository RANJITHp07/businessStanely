...
export async function PUT(
  ...
  try {
    ...
    if (currentAdmin.adminType !== "owner") {
      return NextResponse.json(
        { error: "Only owners can reject categories" },
        { status: 403 }
      );
    }
    ...
    // Find category and ensure it exists
    ...
    // Update category to rejected status
    ...
    console.error("Error rejecting task category:", error);
    return NextResponse.json(
