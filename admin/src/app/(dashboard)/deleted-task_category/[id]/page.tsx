...
interface DeletedTaskCategoryDetail {
  category: {
    id: string;
    name: string;
    description: string;
...
export default function DeletedTaskCategoryDetailPage() {
  const params = useParams();
  const router = useRouter();
  const id = params?.id as string;

  const [data, setData] = useState<DeletedTaskCategoryDetail | null>(null);
  ...
  const {
    ...
    setItemsPerPage,
    clampToTotalPages,
  } = useTablePage("admin-deleted-task_category-tasks", 10);

  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const response = await fetchWithAuth(
          `/api/deleted-task-categories/${id}`
        );
        if (!response.ok) {
          const body = await response.json().catch(() => ({}));
          setError(body.error ?? "Failed to load deleted service");
          return;
        }
        setData(await response.json());
      } catch (err) {
        console.error("Error fetching deleted service:", err);
        setError("Failed to load deleted service");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <section className="container mx-auto p-6 max-w-7xl space-y-4">
        ...
        <div className="flex items-center gap-2 text-destructive text-sm">
          ...
          {error ?? "This deleted service could not be found."}
        </div>
      </section>
    );
  }
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 md:mb-4 gap-4">
          <div>
            ...
            <p className="text-[18px] md:text-[16px] text-muted-foreground mt-2">
              Tasks hidden when this service was deleted
            </p>
          </div>
          <Button asChild variant="outline" className="w-fit">
            <Link href="/task_category">
            ...
            <div className="flex flex-col md:flex-row items-start justify-between gap-6">
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground pt-1">
                  <span className="flex items-center gap-1">
                    ...
                    {category.createdByType === "agent" && " (Agent)"}
                  </span>
                  {category.timePeriod != null && (
                    <span className="flex items-center gap-1">
                      <Building2 className="h-4 w-4" />
                      {category.timePeriod} days
                    </span>
                  )}
              ...
              <div className="text-sm space-y-1 md:text-right">
                <div className="text-muted-foreground">
                  Deleted {formatDateTime(category.deletedAt)}
                </div>
                {category.deletedBy && (
                  <div className="text-muted-foreground">
                    by {category.deletedBy}
                    {category.deletedByType === "AGENT" && " (Agent)"}
                  </div>
                )}
                <div className="text-xs text-muted-foreground pt-1">
                  Status at deletion: {category.status}
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <CardContent className="p-3 sm:p-6">
          {tasks.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              No tasks were hidden with this service.
            </div>
          ) : (
            <Tabs
              value={taskStatus}
                              ...
                              <TableCell className="align-top">
                                <Badge
                                  ...
                                  className={`text-xs whitespace-nowrap ${
                                    STATUS_COLORS[task.status?.toLowerCase()] ??
                                    ""
                                  }`}
                                >
                                  {task.status}
