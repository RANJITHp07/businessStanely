...
export async function GET(req: NextRequest) {
  try {
    ...
    if (agent.agentType === "Lead Maker") {
      opportunities = allOpportunities.filter(o => o.prospect?.createdByAgentId === agent.id);
    } else if (agent.agentType === "Client Advisor" || agent.agentType === "Client Manager") {
      opportunities = allOpportunities.filter(o => o.prospect?.assignedAgentId === agent.id);
    }
