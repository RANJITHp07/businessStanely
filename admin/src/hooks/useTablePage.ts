...
export function useTablePage(key: string, defaultItemsPerPage = 20) {
     ...
     * filtered out while the user was on another screen. A totalPages of 0 means
     * the list is still loading or genuinely empty, which is not a reason to move
     * off the restored page.
     */
    const clampToTotalPages = useCallback((totalPages: number) => {
        if (!restoredRef.current || totalPages <= 0) return
        setCurrentPageState((prev) => (prev > totalPages ? totalPages : prev))
    }, [])
