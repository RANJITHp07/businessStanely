...
export default function ClientsTable() {
  ...
  const [selectedCommunication, setSelectedCommunication] =
    useState("All Communication");
  const [selectedEntityType, setSelectedEntityType] =
    useState("All Entity Types");
  ...
  const filteredClients = clients.filter((client) => {
    ...
    const matchesType =
      ...
      client.clientType === selectedType.toLowerCase();

    const matchesCommunication =
      selectedCommunication === "All Communication" ||
      (client.preferredCommunication &&
        client.preferredCommunication.toLowerCase() ===
          selectedCommunication.toLowerCase());
  ...
  const getCommunicationBadge = (communication: string) => {
    ...
    return (
      <Badge
        className={
          colors[communication as keyof typeof colors] ||
          "bg-gray-100 text-gray-800"
        }
      >
        {communication.charAt(0).toUpperCase() +
          communication.slice(1).replace("-", " ")}
      </Badge>
    );
  };
      ...
      <div className="mb-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Filter and search through your clients
            </CardDescription>
          </CardHeader>
                                ...
                                client.authorizedPersonName && (
                                  <>
                                    ...
                                    {client.authorizedPersonName
                                      .charAt(0)
                                      .toUpperCase() +
                                      client?.authorizedPersonName?.slice(1)}
                                  </>
                                )}
                              {client.clientType === "individual" &&
                                client.gender && (
