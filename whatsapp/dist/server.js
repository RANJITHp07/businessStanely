...
function scheduleClientRecovery(trigger) {
    if (recoveryTimer) {
        return;
    }
    ...
    recoveryTimer = setTimeout(() => {
        ...
        whatsappService.ensureInitialized().catch((err) => {
...
app.get("/stream", auth, async (req, res) => {
    ...
    const unsubState = subscribeWhatsAppEvent("state", () => {
        void (async () => {
            try {
                ...
                console.error("[WhatsApp Backend] Failed to publish stream state:", error);
            }
        })();
    });
    ...
    const unsubMessage = subscribeWhatsAppEvent("message", (payload) => {
        send("message", payload);
    });
    const unsubMessages = subscribeWhatsAppEvent("messages-updated", (payload) => {
