...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { name, email, phone, phoneNumber, address, leadSource, description, status, notes, nextFollowUp, assignedAgentId, amount } = body;
    ...
    if (status === "Opportunity") {
      try {
        ...
        console.error("Error creating opportunity and archiving prospect:", opportunityError);
        return NextResponse.json({ error: "Failed to create opportunity or archive prospect", details: opportunityError?.message || opportunityError }, { status: 500 });
      }
    }
    ...
    return NextResponse.json({ error: "Failed to update prospect", details: error?.message || error }, { status: 500 });
  }
}
